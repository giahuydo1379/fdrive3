...
function rename_file($old_path, $name, $transliteration)
{
    ...
    if (file_exists($old_path))
    {
        ...
        if (file_exists($new_path) && $old_path == $new_path)
            return false;
        //do hàm rename nếu tên mới ($new_path) đả tồn tại thì ($new_path) sẽ bị xóa
        //nên ta sẽ kiểm tra nếu tồn tại thì ta thêm _i vào 
        if (file_exists($new_path))
        {
            $info_new_path = pathinfo($new_path);
            $i = 1;
            $new_path = $info['dirname'] . "/" . $name . "_" . $i . '.' . $info['extension'];
            while (file_exists($new_path)) {
                $i++;
                $info_file = pathinfo($new_path);
                $new_path = $info_file['dirname'] . "/" . $name . "_" . $i . '.' . $info_file['extension'];
            }
        }
...
function setRemoveFontVi_urlTitle($str)
{
